...
Desktop launcher for the Aetherion client (Minecraft 1.21.1, Fabric 0.19.5). Version 0.3.7 keeps the 0.3.4 window, dashboard, and settings. Play installs the Fabric pack, signs in with Microsoft, and connects to play.donnernet.de.
...
```text
dist/Aetherion.Launcher.Setup.0.3.7.exe
```
...
GitHub Actions (`Build Windows Release`) uses `pnpm build:win:ci` on tag `v0.3.7`. The release assets are the installer, its blockmap, and `latest.yml`.

Install `Aetherion.Launcher.Setup.0.3.7.exe` once. After that, newer versions download and install from inside the app. Do not keep replacing the setup file by hand.
